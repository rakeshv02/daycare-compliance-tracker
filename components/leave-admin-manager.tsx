...
function AccessRow({ person, employeeId: initialEmployeeId, enabled, busy, run }: { person: StaffMember; employeeId: string; enabled: boolean; busy: boolean; run: (task: () => Promise<void>, success: string) => void }) {
  ...
  return <div className="flex flex-col gap-3 py-4 lg:flex-row lg:items-end"><div className="min-w-0 flex-1"><b className="text-sm">{person.name}</b><p className="text-xs text-[#74746E]">{person.site} · {enabled ? "PIN active" : employeeId ? "Employee creates PIN at first login" : "Employee ID needed"}</p></div><label className="text-xs text-[#55554F]">Employee ID<input value={employeeId} onChange={(e) => setEmployeeId(e.target.value.toUpperCase().replace(/[^A-Z0-9-]/g, "").slice(0,20))} placeholder="Example: 10024" className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm lg:w-44" /></label><label className="text-xs text-[#55554F]">Reset PIN (optional)<input value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, "").slice(0,8))} inputMode="numeric" type="password" placeholder={enabled ? "Leave empty" : "Employee will create it"} className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm lg:w-40" /></label><button disabled={busy || employeeId.length < 3 || (!!pin && pin.length < 4)} onClick={() => run(async () => { await saveStaffPortalAccess(person.id, employeeId, pin); setPin(""); }, `Portal access saved for ${person.name}.`)} className="flex items-center justify-center gap-1 rounded-xl bg-[#1F4D47] px-3 py-2 text-sm font-semibold text-white disabled:opacity-40"><KeyRound size={14} /> Save</button></div>;
}
